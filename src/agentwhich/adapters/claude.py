    ...
    def resolve(self, repo: Path, cwd: Path, target: Path | None = None) -> ResolutionResult:
            ...
            for path in sorted(rules_root.rglob('*.md')):
                result.layers.append(
                    layer(
                        ...
                        phase='lazy',
                        scope='rule',
                        reason='Claude rule candidate; applicability may depend on target path',
                    )
                )
                ...
                if descendant.is_file():
                    result.layers.append(
                        layer(
                            ...
                            scope='descendant',
                            reason=(
                                'descendant instruction candidate; loaded when Claude works in that subtree'
                            ),
                        )
                    )
        ...
        if target is not None:
            result.warnings.append(
                'Target-aware Claude path rule evaluation is planned for v0.2; '
                'candidates are shown conservatively in v0.1.'
            )
        return result
